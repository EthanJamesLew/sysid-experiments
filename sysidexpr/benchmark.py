  ...
  def load_trajectories(self):
    ...
    def extract_time_states(df):
      ...
      return atraj.Trajectory(
          times=times[idxs], states=states[idxs], inputs=None, state_names=self.config.states
          )
    ...
    for groupname in self.config.groups:
      for sid, s_df in subjects_dfs:
        trajectories[groupname][sid] = extract_time_states(s_df.loc[s_df[groupname]==True])
    
    return {
        g: atraj.TrajectoriesData(ts) for g, ts in trajectories.items()
    }
  
  @staticmethod
  def score_benchmark(
        prediction: PredictionConfiguration, 
        scoring_fcn: Callable[[atraj.TrajectoriesData, atraj.TrajectoriesData], Tuple[Metric, float]],
        test_group = "Test"
    ) -> PredictionResult:
    ...
    pred_traj = Benchmark(BenchmarkConfiguration(
