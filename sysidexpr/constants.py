...
predictions_base_path = pathlib.Path('./drive/Shareddrives/Collaboration with Google/Predictions')


# results path
scores_base_path = pathlib.Path('./drive/Shareddrives/Collaboration with Google/Results/Scores')


def validate_consts():
  """validates the constants"""
  assert os.path.isdir(data_base_path), f"path {data_base_path} is not a directory"
  assert os.path.isdir(predictions_base_path), f"path {predictions_base_path} is not a directory"
  assert os.path.isdir(scores_base_path),  f"path {scores_base_path} is not a directory"
...
plasma_config = BenchmarkConfiguration(
  name = "plasma",
  data_csv = data_base_path / "Plasma data" / "Annotated" /  "Goth_Plasma_WRAP_dem_pacc_082022_annotated.csv",
